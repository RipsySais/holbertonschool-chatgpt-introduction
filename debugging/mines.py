        ...
        while True:
            ...
            try:
                ...
                if not (0 <= x < self.width and 0 <= y < self.height):
                    print("Coordonnées invalides. Essayez de nouveau.")
                    continue
                if not self.reveal(x, y):
                    self.print_board(reveal=True)
                    print("Game Over! Vous avez touché une mine.")
                    break
                # Vérifiez si toutes les cases non minées sont révélées
                if all(self.revealed[y][x] or (y * self.width + x) in self.mines for y in range(self.height) for x in range(self.width)):
                    self.print_board(reveal=True)
                    print("Congratulations! You've won the game.")
                    break
            except ValueError:
                print("Entrée invalide. Veuillez entrer uniquement des numéros.")
            except KeyboardInterrupt:
                print("\nInterruption du jeu. Au revoir!")
                break
